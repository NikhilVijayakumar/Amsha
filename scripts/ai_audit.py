...
def load_llm_via_factory(llm_config_path: Path) -> Optional[Any]:
    """Load LLM using Amsha's llm_factory"""
    if not HAS_AMSHA:
        print_error("Amsha modules not importable. Make sure you're in the project root.")
        return None
...
def construct_system_prompt(agents_md: str, deps_md: Optional[str] = None) -> str:
    ...
    prompt = f"""You are a Senior Architect reviewing code changes for the Amsha library.
