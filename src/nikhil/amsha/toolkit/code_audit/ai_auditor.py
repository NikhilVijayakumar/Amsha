...
from nikhil.amsha.toolkit.code_audit.config import AuditConfig
...
class AIAuditor:
    ...
    def _create_file_review_prompt(self, file_path: Path, content: str) -> str:
        ...
        return f"""Review Python file for architectural compliance:
...
JSON format."""
    ...
    def _construct_system_prompt(self, rules: str, dependencies: Optional[str] = None) -> str:
        ...
        prompt = f"""You are a Senior Architect reviewing code changes.
...
"""
        ...
        prompt += """
Focus on:
1. Architecture: Dependencies, layer separation
2. Interfaces: ABC vs Protocol usage
3. Dependency Injection: Injected vs instantiated
4. Exceptions: Custom vs generic
5. Type Hints: Complete coverage
6. Framework Isolation: Adapters vs direct usage
7. Dependency Management: pyproject.toml vs requirements.txt
...
Provide JSON review."""
    ...
    def _review_chunked(self, diff: str, system_prompt: str) -> AuditResult:
        ...
        for i, chunk in enumerate(chunks):
            ...
            user_prompt = f"""Review chunk {i+1} ({len(chunk)} files):
...
JSON review:"""
    ...
    def audit(self) -> AuditResult:
        ...
        if not rules:
            print("⚠️  No rules file found, using basic checks")
            return self._basic_checks(diff)
...
JSON review:"""
    ...
    def audit(self) -> AuditResult:
        ...
        if self._load_llm():
            ...
            return self._review_with_llm(diff, rules, dependencies)
        else:
            print("⚠️  LLM not available, using basic checks")
            return self._basic_checks(diff)
