"""
Command-line interface for Amsha AI Code Audit
"""
...
from nikhil.amsha.toolkit.code_audit.config import AuditConfig, resolve_config
from nikhil.amsha.toolkit.code_audit.ai_auditor import AIAuditor
...
def print_header():
    ...
    print(f"{Colors.HEADER}{Colors.BOLD}  🤖 Amsha AI Architecture Audit{Colors.ENDC}")
...
Examples:
  ...
  # Specify all configs
  amsha-audit --rules AGENTS.md --config llm_config.yaml --dependencies DEPS.md

  # Skip audit
  amsha-audit --skip

Environment Variables:
  AMSHA_RULES_FILE       - Path to rules file (e.g., AGENTS.md)
  AMSHA_LLM_CONFIG       - Path to LLM config YAML
  AMSHA_DEPENDENCIES_FILE - Path to dependencies file
...
Config File Discovery:
  ...
  5. Package defaults
    ...
    parser.add_argument(
        '--version',
        action='version',
        version='%(prog)s 1.5.3 (Amsha Code Audit)'
    )
