# Functional Documentation: Amsha Library
...
> **Amsha** aims to solve these problems by providing a lightweight, flexible framework that establishes the file system or database as the source of truth, enabling developers to manage their CrewAI configurations with the same rigor and tooling (e.g., Git) as their application code.
...
-   **FR-CREW-05: Multi-Crew Pipeline via Flow:** A `job_config.yaml` `pipeline` (ordered list of crew names) is executed as a CrewAI `Flow` by `AmshaCrewFileApplication.run_pipeline()`. Each pipeline step delegates to the backing orchestrator's `run_crew(..., INTERACTIVE)` — so every crew still gets its execution state, monitoring, and (if configured) checkpoint recording — and results accumulate in a typed `PipelineState.outputs` keyed by crew name. INTERACTIVE mode blocks and returns the outputs dict; BACKGROUND submits the whole flow to the `RuntimeEngine` and returns an `ExecutionHandle`. Single-crew callers keep using `orchestrator.run_crew()` unchanged; the Flow path is additive.
-   **FR-CREW-06: Agent Tool Resolution:** An agent's YAML *may* list `tools: [tool_name, ...]`. Each name is resolved against the tool registry (`tool_registry.py`) — a name→`BaseTool` mapping that ships with built-in tools (`file_read`, `directory_read`, `scrape_website`) and can be extended via `register_tool()`. Unknown names raise `ValueError` at build time. Resolved tools are passed to `Agent(tools=[...])`.
