...
Amsha enforces **Strict Configuration Validation**. It requires users to pass complete config dictionaries/YAML files at runtime. It **does not fallback to risky defaults**. If a property is missing (like a `base_url` for a local model, or a `domain_root_path`), Amsha throws an `AmshaConfigurationException` right at initialization. 
