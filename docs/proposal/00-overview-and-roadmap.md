...
| | |
|---|---|
| **Status** | All phases done (2026-09-02); see below |
| **Date** | 2026-09-02 |
...
6. **[03 — Flows adoption](03-flows-adoption.md)** and **[10 — Production architecture alignment](10-production-architecture-alignment.md)** — the bigger architectural shift; do this once the smaller pieces are stable, since Flow-first changes how `BaseCrewOrchestrator` fits in. **✅ Done (2026-09-02), together.** Added `FlowCrewOrchestrator` + `PipelineState` (`orchestrator/flow/`) — the previously-dead `job_config.yaml` `pipeline` key is now executed as a CrewAI Flow whose steps delegate to `run_crew()` (keeping execution state, monitoring, and the proposal-05 checkpoint wiring), plus additive `AmshaCrewFileApplication.run_pipeline()`. `BaseCrewOrchestrator`/`FileCrewOrchestrator` unchanged for single-crew callers. Proposal-10 gaps were closed mostly by 03/05/08; LLM Hooks remain explicitly deferred (see 10 log).
7. **[09 — Event observability upgrade](09-event-observability-upgrade.md)** — replace/augment `CrewPerformanceMonitor` once Flows/Memory land, since new event categories (memory ops, flow methods) only exist once those are in use. **✅ Done (2026-09-02).** `AmshaEventListener` (BaseEventListener subclass) subscribes to crew/task/LLM/tool/flow/flow-method lifecycle events on the bus, logs them through Amsha's existing `MetricsLogger` pipeline, and correlates paired start/end events via `started_event_id` for per-task/per-call/per-method durations + per-call tokens. Instantiated at import in `crew_monitor/__init__.py` (module-level ref avoids the GC footgun). `CrewPerformanceMonitor`'s psutil/pynvml sampling stays as the OS-level complement.
8. **[11 — Agent/Task crafting skill](11-agent-task-crafting-skill.md)** — can actually be done *first* or in parallel with anything above; it's a Claude Code skill, not a code change to Amsha, and is most useful once (3) gives it real fields to work with. **✅ Done (2026-09-02).** `.claude/skills/craft-amsha-agent/SKILL.md` adds quick / `--interview` / `review` modes, the CrewAI crafting heuristics + pitfalls checklist as a lint table, Amsha-convention YAML templates, and an opt-in recommendation step for the proposal-08 fields.
